...
const TargetMonsterSelect: React.FC<TargetMonsterSelectProps> = ({
  disabled,
}) => {
  const { pockestState, pockestDispatch } = usePockestContext();
  const targetableMonsters = React.useMemo(
    () => pockestGetters.getCurrentTargetableMonsters(pockestState),
    [
      pockestState,
    ],
  );
  const curMonsterNonHatchLogs = React.useMemo(
    () =>
      pockestGetters
        .getCurrentMonsterLogs(pockestState)
        .filter((l) => l.logType !== 'hatching'),
    [
      pockestState,
    ],
  );
    ...
    <select
      ...
      onChange={(e) => {
        ...
        const shouldAbort =
          shouldConfirm &&
          !window.confirm(
            'Are you sure you want to change the target monster? Doing so at this time may result in unexpected results such as death or failure to evolve.',
          );
        if (shouldAbort) {
