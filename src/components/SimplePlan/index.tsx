...
const SimplePlan: React.FC<SimplePlanProps> = ({ rows = 12 }) => {
  const textAreaEl = React.useRef<HTMLDivElement>(null);
  const { pockestState } = usePockestContext();
  const schedule = React.useMemo(
    () =>
      pockestGetters.getPlanLog(pockestState).map((d) => ({
        ...d,
        icon: (() => {
        ...
        dateStr: new Date(d.start).toLocaleString([], {
          year: 'numeric',
          month: 'numeric',
          day: 'numeric',
        }),
        timeStr: new Date(d.start).toLocaleString([], {
          hour: 'numeric',
          minute: 'numeric',
        }),
      })),
  ...
  const scheduleLinesGrouped: Record<string, (typeof schedule)[0][]> =
    React.useMemo(
      () =>
        schedule.reduce(
          (groups: Record<string, (typeof schedule)[0][]>, d) => ({
            ...groups,
            [d.dateStr]: [
              ...(groups[d.dateStr] || []),
              d,
  ...
  React.useEffect(() => {
    ...
    const lastSuccessIndex = schedule?.reduce(
      (latestIndex, item, itemIndex) =>
        item?.completion ? itemIndex : latestIndex,
      0,
    );
      ...
      <div className="SimplePlan-content">
        {Object.keys(scheduleLinesGrouped).map((dateStr) => (
          <div key={dateStr} className="SimplePlan-day">
            <h1
              className={cx('SimplePlanDay', {
                'SimplePlanDay--completion': !scheduleLinesGrouped[
                  dateStr
                ].find((d) => !d.completion && !d.missed),
              })}
            >
              {dateStr}
            </h1>
            {scheduleLinesGrouped[dateStr].map((d) => (
              <div
                ...
                className={cx('PockestLine', 'SimplePlanEvent', {
              ...
              >
                ...
                <p className="SimplePlanEventTime PockestToolTip PockestToolTip--left PockestToolTip--bottom">
                  ...
                  <span className="PockestToolTip-text">
                    All times are aproximations. Actual task execution times may
                    vary by a few minutes and this shouldn&apos;t affect the
                    final outcomes.
                  </span>
                </p>
              </div>
