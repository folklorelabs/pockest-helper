...
function TrainControls() {
  ...
  const { data, autoPlan, autoTrain, paused, stat } = pockestState;
  return (
    <div className="TrainControls">
      <div className="PockestLine">
        ...
        {Object.keys(STAT_ID).map(
          (k) =>
            k !== '0' && (
              <span
                key={k}
                className={`PockestStat PockestStat--${STAT_ID[k]}`}
              >
                ...
                <span className="PockestStat-value">
                  {typeof data?.monster?.[
                    STAT_ID[k] as keyof typeof data.monster
                  ] === 'number'
                    ? data?.monster?.[STAT_ID[k] as keyof typeof data.monster]
                    : '--'}
                </span>
              </span>
            ),
