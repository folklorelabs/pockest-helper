...
const CareLog: React.FC<CareLogProps> = ({
    ...
    if (onlyDiscoveries) {
      return d.filter(
        (entry) =>
          ...
          pockestGetters.isMatchDiscovery(pockestState, entry),
      );
    }
    return d;
  }, [
  ...
  const careLog = React.useMemo(
    () => [
      `[${APP_NAME}]`,
      ...careLogData.map((entry) =>
        (onlyDiscoveries ? getMatchReportString : getActionResultString)({
          pockestState,
          result: entry,
          isRelTime,
        }),
    ...
    <div className="CareLog">
      <header className="CareLog-header">
        <p className="CareLog-title">
          {title} ({careLogData?.length || 0})
        </p>
        {!onlyDiscoveries ? (
          <label
            className="PockestCheck"
            htmlFor={`PockestHelper_CareLogAbsTime--${title.replace(' ', '')}`}
          >
            <input
            ...
            <button
              ...
              onClick={() => {
                const confirm = window.confirm(
                  `Are you sure you want to permanently delete your old ${title.toLowerCase()} entries? This will include all entries before your last egg hatch.`,
                );
                if (!confirm) return;
                if (pockestDispatch)
                  pockestDispatch(
                    pockestActions.pockestClearLog(pockestState, logTypes),
                  );
              }}
            >
              ❌ Clean
