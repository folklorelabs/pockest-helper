...
function Memento() {
  ...
  const mementoImg = React.useMemo(() => {
    ...
    const mementoHash = pockestState?.allHashes
      ?.filter((h) => h.type === 'memento')
      .find((h) => h?.id?.includes(`${curMonId}`));
    ...
    pockestState?.allHashes,
    pockestState?.data?.monster?.hash,
  ]);
  const { memento_point = 0, max_memento_point = 0 } =
    pockestState?.data?.monster || {};
  return (
    <span
      className={cx('PockestMemento', {
        'PockestMemento--missingImg': !mementoImg,
      })}
      style={{
        backgroundImage: mementoImg ? `url(${mementoImg})` : 'none',
      }}
    >
