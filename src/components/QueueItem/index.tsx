...
function QueueItemInner({
  ...
  const { editMode, setEditMode } = React.useContext(QueueItemContext);
  ...
  const planEgg = React.useMemo(
    () =>
      presetQueueItem?.monsterId === -1
        ...
        : pockestGetters.getMonsterEgg(
            pockestState,
            presetQueueItem?.monsterId,
          ),
    [
      pockestState,
      presetQueueItem,
  ...
  const estimatedBalance = React.useMemo(() => {
    const previousCosts = pockestState?.presetQueue
      ?.slice(0, queueIndex)
      .reduce((sum, item) => {
        const egg =
          item.monsterId === -1
  ...
  const canAffordEgg = React.useMemo(
    () =>
      planEgg &&
      (planEgg.unlock ||
        (planEgg?.buckler_point || Infinity) <= estimatedBalance),
    [
      planEgg,
      estimatedBalance,
    ],
  ...
  return (
    <div className="QueueItem">
      ...
      <button
        ...
        disabled={
          queueIndex < editableStartIndex ||
          queueIndex >= pockestState?.presetQueue?.length - 1
        }
        onClick={() => {
          if (!pockestDispatch) return;
          const presetQueue: PresetQueueItem[] = [
        ...
        <>
          <span className="PockestToolTip PockestToolTip--right PockestToolTip--bottom">
            ...
            <span className="PockestToolTip-text">
              Your <em>Buckler point</em> balance may be insufficient to cover
              this egg purchase ({planEgg?.buckler_point}) once it is reached in
              the queue.
              <br />
              This is based on your estimated balance at this point in the queue
              ({Math.max(0, estimatedBalance ?? 0)}).
            </span>
          </span>{' '}
        </>
      )}
      {editMode ? (
        ...
        <>
          <span className="QueueItemLabel">
            {pockestGetters.getPresetQueueItemLabel(
              pockestState,
              presetQueueItem,
            )}
          </span>
