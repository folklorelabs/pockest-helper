...
import type PresetQueueItem from "../types/PresetQueueItem";
...
export function getLogEntry(
	...
	return {
		...
		monsterId: mergedData?.monster?.hash
			? getMonsterIdFromHash(mergedData?.monster?.hash)
			: -1,
...
export function getPresetQueueItemLabel(
	state: PockestState,
	presetQueueItem?: PresetQueueItem | null,
) {
	if (!presetQueueItem) return "";
	const monster = state.allMonsters.find(
		(m) =>
			presetQueueItem.monsterId && m.monster_id === presetQueueItem.monsterId,
	);
	if (!monster?.name_en)
		return `${presetQueueItem.planId}${presetQueueItem.statPlanId ? `-${presetQueueItem.statPlanId}` : ""}`;
	const planAge =
		presetQueueItem.monsterId !== -1
			? presetQueueItem.planAge
			: parsePlanId(presetQueueItem.planId)?.planAge;
...
export function getCurrentTargetableMonsters(
	state: PockestState,
	targetAge: number = 6,
) {
	...
	const allAvailIds = state?.allMonsters
		...
		.reduce(
			(all, m) => {
				...
				const match = m.from.find(
					(pid) => pid === curMonsterId || all.includes(pid),
				);
				if (!match) return all;
				return [...all, m.monster_id].filter((id) => typeof id === "number");
			},
...
export async function getBestMatch(
	state: PockestState,
	exchangeList: BucklerPotentialMatch[],
) {
	const monsterId = getMonsterId(state);
	const monster = state?.allMonsters?.find((m) => m.monster_id === monsterId);
	const sortedMatches = exchangeList
		?.map((a) => {
...
export function getRequiredMementoMonsterIds(
	state: PockestState,
	excludeAcquired?: boolean,
) {
	const excludedIds = excludeAcquired ? getOwnedMementoMonsterIds(state) : [];
	return (
		state?.allMonsters
			?.filter(
				(m) => m?.requiredMemento && !excludedIds.includes(m?.requiredMemento),
			)
...
export function getOwnedMementoMonsterNames(state: PockestState) {
	...
	return mementosOwned.map(
		(id) => state.allMonsters?.find((m) => m.monster_id === id)?.name_en,
	);
}
...
export function getTargetMonsterPlanId(state: PockestState) {
	const monster = state?.allMonsters?.find(
		(m) => m.monster_id === state?.monsterId,
	);
	const planId = (() => {
		...
		return LEGACY_PLAN_REGEX.test(id)
			? `${id.substring(0, 1)}${state?.planAge}${id.substring(2)}`
...
export function getTargetMonsterStatPlanId(state: PockestState) {
	...
	const { primaryStatLetter } = parsePlanId(statePlanId) ?? {};
	const statPlanId = (() => {
		if (monster?.statPlan) return monster.statPlan;
		const fallbackStatPlanId = Array.from(new Array(6))
			.map(() => primaryStatLetter)
			.join("");
...
export function getTargetMonsterPlan(state: PockestState) {
	...
	const { planId, planEgg, planRoute, primaryStat, planAge } =
		parsePlanId(statePlanId) ?? {};
	const statPlan = statPlanId
		.split("")
		.map((statLetter) => STAT_ABBR[statLetter]);
	return {
		planId,
		primaryStat,
		statPlanId,
...
export function getTargetMonsterCurrentRouteSpec(state: PockestState) {
	...
	const currentRouteSpec = targetPlan?.planRoute?.find(
		(r) => curAge >= r?.ageStart && curAge < r?.ageEnd,
	);
...
export function getTargetMonsterNextStat(state: PockestState) {
	...
	const stat = (() => {
		...
		const curTrainings = state?.log?.filter(
			(entry) =>
				state?.data?.monster?.live_time &&
				entry.timestamp > state?.data?.monster?.live_time &&
				entry.logType.includes("training"),
		);
		const numTrains = Math.max(state?.statLog?.length, curTrainings?.length);
		return parseInt(
			targetPlan?.statPlan?.[numTrains] ?? targetPlan?.primaryStat,
			10,
		);
	})();
	return stat;
...
export function getCareSettings(state: PockestState) {
	if (state?.autoPlan) {
		...
		if (!routeSpec) return {};
		const { cleanFrequency, feedFrequency, feedTarget } = routeSpec;
		return {
			cleanFrequency,
			feedFrequency,
			feedTarget,
...
export function getPlanNeglectOffset(state: PockestState) {
	let ageOffset =
		state?.planAge && state?.planAge > 1
			? MONSTER_AGE[Math.max(2, state.planAge)]
			: 0;
	if (state.planAge === 5) {
...
export function getPlanStunOffset(state: PockestState) {
	...
	const ageOffset =
		state?.planAge && state?.planAge > 1
			? Math.max(
					0,
					MONSTER_AGE[Math.max(2, state.planAge)] - 5 * 60 * 60 * 1000,
				)
			: 0;
	return ageOffset;
}
...
export function getCurrentPlanSchedule(state: PockestState) {
	...
	const cleanSchedule =
		state?.autoPlan && targetPlan?.planRoute
			? targetPlan.planRoute.reduce((fullSchedule, spec, index) => {
					if (!spec) return fullSchedule;
					if (spec.startTime > neglectOffset) return fullSchedule;
					const start = birth + spec.startTime;
					const end =
						spec.endTime > neglectOffset
							? birth + neglectOffset
							: birth + spec.endTime;
					const schedule =
						getTimeIntervals(
	...
	const feedSchedule: FeedInterval[] =
		state?.autoPlan && targetPlan?.planRoute
			? targetPlan.planRoute.reduce((fullSchedule, spec) => {
					if (!spec) return fullSchedule;
					if (spec.startTime > neglectOffset) return fullSchedule;
					const start = birth + spec.startTime;
					const end =
						spec.endTime > neglectOffset
							...
							: birth + spec.endTime;
					const schedule = (
						getTimeIntervals(start, end, spec.feedFrequency, spec.feedOffset) ||
						[]
					).map((s) => ({
						...s,
...
export function getMatchSchedule(state: PockestState) {
	...
	const firstMatchTime =
		(state?.planAge < 4 ? birth : getFirstMatchTime(state)) ?? birth;
	const numMatches = Math.max(
		0,
		Math.ceil((birth + targetDeath - firstMatchTime) / daysToMs(1)),
	);
...
export function getCurrentPlanScheduleWindows(state: PockestState) {
	...
	const now = Date.now();
	const nextCleanWindow =
		cleanSchedule &&
		cleanSchedule?.find((scheduleWindow) => now < scheduleWindow.start);
	const currentCleanWindow =
		cleanSchedule &&
		cleanSchedule?.find(
			(scheduleWindow) =>
				now >= scheduleWindow.start && now <= scheduleWindow.end,
		);
	const nextFeedWindow =
		feedSchedule &&
		feedSchedule?.find((scheduleWindow) => now < scheduleWindow.start);
	const currentFeedWindow =
		feedSchedule &&
		feedSchedule?.find(
			(scheduleWindow) =>
				now >= scheduleWindow.start && now <= scheduleWindow.end,
...
export function isMonsterDeparted(
	state: PockestState,
	data?: BucklerStatusData | null,
) {
	if (data?.event === "hatching") return false;
	...
	const birthTimestamp =
		state?.eggTimestamp === monster?.live_time
			? state?.eggTimestamp
			: monster?.live_time;
	return birthTimestamp && now >= birthTimestamp + daysToMs(7);
}

export function isMonsterMissing(
	_state: PockestState,
	data?: BucklerStatusData | null,
) {
	if (data?.event === "hatching") return false;
...
export function getPlanEvolutions(
	state: PockestState,
): Record<string, Monster> {
	if (!state?.planId) return {};
	const parsedPlanId = parsePlanId(state.planId);
	if (!parsedPlanId) return {};
	const { planEgg, planRouteId, primaryStatLetter, planAge } = parsedPlanId;
	const numEvolutions = Math.max(2, Math.min(5, planAge));
	const eggMonsters = state.allMonsters.filter((m) =>
		m?.eggIds?.includes(planEgg),
	);
	const planEvolutions = Array.from(new Array(numEvolutions)).reduce(
		(acc, _val, index) => {
			const fromMon = acc[index];
			const age = index + 1;
			const matchingMonsters = eggMonsters.filter(
				(m) =>
					m.age === age && (!fromMon || m.from.includes(fromMon.monster_id)),
			);
			const matchingIndex = (() => {
				if (age === 3) {
...
export function getPlanLog(state: PockestState) {
	...
	const { cleanSchedule, feedSchedule } = planSchedule;
	...
	data = [
		...
		...Object.keys(MONSTER_AGE)
			...
			.filter(
				(age) => age > 1 && age <= Math.max(2, Math.min(5, state?.planAge)),
			)
			.map((age) => ({
				logType: "evolution",
				logGrace: 1000 * 60 * 60,
				...
				completion: planEvolutions
					? !!getCurrentMonsterLogs(state, ["evolution"]).find(
							(l) => l.monsterId === planEvolutions?.[age]?.monster_id,
						)
					: null,
			})),
		...(cleanSchedule?.map((w) => ({
		...
		.map((w: PlanLogEntry) => {
			const completion =
				w.completion ??
				!!getCurrentMonsterLogs(state, [w.logType || ""]).find(
					(l) =>
						l?.timestamp >= w.start &&
...
export function isConfirmedMonster(
	state: PockestState,
	data: BucklerStatusData,
) {
	const matchingMonster = state.allMonsters.find(
		(m) =>
			data?.monster?.hash &&
...
export async function getDiscordReportEvoSuccess(
	state: PockestState,
	data: BucklerStatusData,
) {
	...
	const statLogStr = state?.statLog
		?.map((s) => `${STAT_ID_ABBR[s]}`)
		?.slice(0, 6)
		?.join("");
	const statLog = `Training Log: ${statLogStr ? `**${statLogStr}**` : ""}`;
	const statsTotal = data?.monster
		? data.monster.power + data.monster.speed + data.monster.technic
		: 0;
	...
	const charSprites = data?.monster?.hash
		? await fetchCharSprites(data?.monster?.hash)
		: [];
	const idle1Sprite = charSprites?.find((sprite) =>
		sprite?.fileName.includes("idle_1"),
	);
	const embed = {
...
export function getDiscordReportEvoFailure(
	state: PockestState,
	data: BucklerStatusData,
) {
	const mementosOwned = getOwnedMementoMonsterNames(state);
...
export async function getDiscordReportMemento(
	state: PockestState,
	data: BucklerStatusData,
) {
	const encycloData = state?.allMonsters?.find(
...
export function isMatchDiscovery(
	pockestState: PockestState,
	exchangeResult: Partial<BucklerMatchResults>,
) {
	const monster = pockestState?.allMonsters.find(
		(m) =>
			pockestState?.data?.monster?.hash &&
			m.monster_id === getMonsterIdFromHash(pockestState?.data?.monster?.hash),
	);
	const allMissing = [
	...
	return (
		typeof exchangeResult?.target_monster_id === "number" &&
		allMissing.includes(exchangeResult?.target_monster_id)
	);
}

export function getDiscordReportMatch(
	state: PockestState,
	exchangeResult: Partial<BucklerMatchResults>,
	opponentName: string,
) {
...
export async function getDiscordReportSighting(
	_state: PockestState,
	...
	const statsTotal = matchResults
		? matchResults.power + matchResults.speed + matchResults.technic
		: 0;
