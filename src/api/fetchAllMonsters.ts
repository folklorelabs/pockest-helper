...
export default async function fetchAllMonsters(): Promise<Monster[]> {
  const [bucklerData, sheetMonsters, hashes] = await Promise.all([
    fetchBucklerEncyclo(),
    fetchSheetMonsters(),
    fetchAllHashes(),
  ]);
  ...
  const bucklerMonsters: Record<number, BucklerEncycloMonster[]> =
    bucklerData?.data?.books.reduce((allMonsters, book) => {
  ...
  const allMonsters = Object.keys(bucklerMonsters).map((monsterIdStr) => {
    ...
    const bucklerMonster = matchingBucklerMonsters.reduce(
      (combinedMonster, m) => ({
        ...combinedMonster,
        ...m,
        eggIds: m.eggId
    ...
    const hashMonster = hashMonsters?.find((h) =>
      h?.id?.includes(`${monsterId}`),
    );
    return {
      ...bucklerMonster,
      ...sheetMonster,
